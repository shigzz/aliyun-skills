...
class Sample:
    ...
    def main(
        ...
        headers = {}
        try:
            resp = client.list_log_stores_with_options('', list_log_stores_request, headers, runtime)
    ...
    async def main_async(
        ...
        headers = {}
        try:
            resp = await client.list_log_stores_with_options_async('', list_log_stores_request, headers, runtime)
            print(json.dumps(resp, default=str, indent=2))
        except Exception as error:
            # 此处仅做打印展示，请谨慎对待异常处理，在工程项目中切勿直接忽略异常。
            # 错误 message
