...
class Sample:
    ...
    def main(
        ...
        try:
            resp = client.batch_set_cdn_domain_config_with_options(batch_set_cdn_domain_config_request, runtime)
    ...
    async def main_async(
        ...
        runtime = util_models.RuntimeOptions()
        try:
            resp = await client.batch_set_cdn_domain_config_with_options_async(batch_set_cdn_domain_config_request, runtime)
            print(json.dumps(resp, default=str, indent=2))
        except Exception as error:
            # 此处仅做打印展示，请谨慎对待异常处理，在工程项目中切勿直接忽略异常。
            # 错误 message
